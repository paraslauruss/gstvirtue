...
export function ProductImportExport() {
    ...
    const [importLLoading, setImportLoading] = useState(false);

    const handleFileUpload = (event) => {
        const file = event.target.files[0];
        console.log("CSV Data", "File Selected");
        if (!file) return;
    ...
    const handleSubmit = async () => {
        ...
        const formattedData = csvData.map((row) => ({
            id: row.Product_Id,
            gst: parseFloat(row.GST_Percentage) || 0,
            hsn: row.HSN,
            miniAmount: parseFloat(row.Mini_Amount) || 0,
            minGst: parseFloat(row.Mini_GST_Percentage) || 0,
            cess: parseFloat(row.Cess_Gst_Percentage) || 0,
        }));

        setImportLoading(true);
        try {
            const response = await fetch("http://localhost:3001/api/products/update-products", {
                method: "POST",
                headers: {
        ...
        <div style={{ minHeight: "100vh", padding: '50px 100px' }}>
            ...
            <div style={{ display: 'flex', gap: '20px', marginTop: '20px' }}>
                <div style={{ width: '100%' }}>
                    <Card padding={0}>
                        <div style={{ color: 'white', background: '#565656', padding: '10px 20px' }}>
                            <Text variant="headingLg">Steps for Importing Product Data with HSN & GST %</Text>
                        </div>
                        ...
                        <div style={{ padding: '10px 20px' }}>
                            ...
                            <div style={{ marginTop: '10px' }}>
                                <p><b>2. Fill and Validate:</b>  After filling out the exported file, review the validation rules required for importing the CSV file.</p>
                            </div>
                            <div style={{ marginTop: '10px' }}>
                                <p><b>3. Import the CSV File:</b> Import the CSV file. Initially, the system will display your first product for confirmation.</p>
                            </div>
                            <div style={{ marginTop: '10px' }}>
                                <p><b>4. Confirm and Start Import:</b> Click the confirm button to begin the process of importing products with their HSN and GST %.</p>
                            </div>
                            <div style={{ marginTop: '10px' }}>
                                <p><b>5. Error Handling:</b> If there are any errors, they will be displayed. Correct the errors and import the file again.</p>
                            </div>
                            <div style={{ marginTop: '10px' }}>
                                <p><b>6. Successful Import:</b> If there are no errors, please wait a moment as the file is being imported. The process may take some time.</p>
                            </div>
                ...
                <div style={{ width: '100%' }}>
                    <Card>
                        <div >
                            <Text variant="headingLg">Product Import</Text>
                        </div>
                        ...
                        <div style={{ alignItems: 'center', borderRadius: '5px', color: 'black', background: '#E2E2E2', display: 'flex', padding: '5px 10px', marginTop: '20px' }}>
                            ...
                            <span>
                                <Text variant="bodySm">Note: Upload Product CSV File Only.</Text>
                            </span>
                            <span>
                    ...
                    <div style={{ marginTop: '20px' }}>
                        <Card>
                            <div >
                                <Text variant="headingLg">Product Export</Text>
                            </div>

                            <div style={{ alignItems: 'center', borderRadius: '5px', color: 'black', background: '#E2E2E2', display: 'flex', padding: '5px 10px', marginTop: '20px' }}>
                                <img src={ic_info} height={12} style={{ marginRight: '10px' }} />
                                <span>
                                    <Text variant="bodySm">Note: You can export up to 5,000 products at a time. If your store has more than 5,000 products, you can export them in batches by entering the products Id from the first exported CSV file to sontinue exporting the remaining products.</Text>
                                </span>
