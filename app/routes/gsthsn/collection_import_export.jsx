...
export function CollectionImportExport() {
    ...
    const handleDownload = async () => {
        ...
        const apiUrl = 'http://localhost:3001/api/collection';
        const headers = {
        ...
        try {
            ...
            const csvData = result.map((collection) => ({
                Collection_Id: collection.id,
                Collection_Name: collection.title,
                HSN: collection.hsn_code,
                GST_Percentage: collection.gst,
                Mini_Amount: collection.mini_amount,
                Mini_GST_Percentage: collection.mini_gst,
                Cess_Gst_Percentage: collection.cess,
            }));
        ...
        Papa.parse(csvFile, {
            ...
            complete: async (results) => {
                if (results.errors.length) {
                    ...
                    const collections = results.data.map((row) => ({
                        id: row['Collection_Id'],
                        mini_amount: row['Mini_Amount'],
                        mini_gst: row['Mini_GST_Percentage'],
                        gst: row['GST_Percentage'],
                        hsn_code: row['HSN'],
                        cess: row['Cess_Gst_Percentage'],
                    }));

                    const apiUrl = 'http://localhost:3001/api/collection/collections/update';
                    const headers = {
                        'store-name': storeName,
                        'api-version': '2025-01',
                        'access-token': accessToken,
                    ...
                    try {
                        ...
                        const result = await response.json();
                        console.log('Collections updated successfully:', result);
                    ...
                    <Card>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'end', marginTop: '20px' }}>
                            ...
                            >
                                ...
                                <div style={{ marginLeft: '10px' }}><Text>{importLoading ? 'Importing...' : 'Import'}</Text></div>
                            </div>
                        </div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                >
                                    ...
                                    <div style={{ marginLeft: '10px' }}>
                                        <Text>{loading ? 'Exporting...' : 'Export'}</Text>
                                    </div>
                                </div>
                            </div>
                        </Card>
