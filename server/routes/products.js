    ...
    router.post('/', async (req, res) => {
        try {
            ...
            res.status(500).json({ message: 'Failed to process products', error: error.message });
        }
    });
    ...
    router.put('/:id', async (req, res) => {
       ...
       // Validate the ID format - FIX
         if (!productId || !/^\d+$/.test(productId)) {
              return res.status(400).json({ message: 'Invalid product ID format. ID must be a numeric string.' });
         }
